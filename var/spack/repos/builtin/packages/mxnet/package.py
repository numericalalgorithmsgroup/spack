...
class Mxnet(CMakePackage, CudaPackage):
    ...
    variant(
        "opencv", default=True, description="Use OpenCV for vision support"
    )
    variant("mkldnn", default=False, description="Use the MKLDNN extensions")
    variant(
        "python", default=True, description="Build Python (gluon) interface"
    )
    variant(
        "nccl",
        default=True,
        description="Use NVIDIA NCCL communications layer",
    )
    variant(
        "openmp",
        default=False,
        description="Enable OpenMP parallelism support",
    )
    variant(
        "profiler",
        default=False,
        description="Enable builtin profiler support",
    )
    ...
    depends_on(
        "opencv+core+imgproc+highgui+jpeg+png+tiff~eigen~ipp@3.0:",
        when="+opencv",
    )
    ...
    def cmake_args(self):
        args = [
            ...
            self.define_from_variant("-DUSE_NCCL", "nccl"),
        ...
        if "openblas" in self.spec:
            ...
            args.append("-DUSE_LAPACK=False")
        else:
            raise InstallError(
                "Unsupported blas provider - need MKL, Openblas or Atlas"
            )
    ...
    def install(self, spec, prefix):
        ...
        if "+python" in spec:
            ...
            # Need to hold the installer's hand a bit here
            for root, dirs, files in os.walk(prefix):
                if "libmxnet.so" in files:
                    python.add_default_env(
                        "MXNET_LIBRARY_PATH", os.path.join(root, "libmxnet.so")
                    )
                    break
